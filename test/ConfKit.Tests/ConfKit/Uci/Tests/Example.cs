...
config interface
   option ifname 'eth2'";
        ...
        var jsonObject = config.ToSerializableJsonObject();
        var network = jsonObject.Deserialize<NetworkConfig>();
        ...
        var eth1 = network.Interfaces[0];
        Assert.Equal("eth1", eth1.IfName);

        var eth2 = network.Interfaces[1];
